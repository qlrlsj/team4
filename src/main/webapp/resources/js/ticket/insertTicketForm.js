...
$("#insertBtn").on("click", function () {
  //제출시 주소+상세주소
  const businessAddr = $("input[name=businessAddr]").val(
    $("#sample6_address").val() + " " + $("#sample6_detailAddress").val()
  );
  //소요시간 계산
  const hour = $("#hour").val();
  const minute = $("#minute").val();
  $("input[name=requiredTime]").val(hour * 60 + Number(minute));
...
(imageView = function imageView(att_zone, btn) {
  ...
  // 미리보기 이미지 속성
  var img_style =
    "width:100%;height:100%;border:0.3px solid rgb(215, 197, 197);z-index:none";
  // 이미지안에 표시되는 체크박스의 속성
  var chk_style =
    "width:30px;height:30px;line-height:30px;position:absolute;font-size:18px;" +
...
function add_optbox() {
  ...
  //가격 값 입력시 할인된 금액 값 자동변경
  $(".price").keyup(function () {
    const rate = $(this).next().next().next().val();
    const price = $(this).val();
    const discountPrice = $(this).next().next().next().next().next();
    discountPrice.val(Math.ceil(price * (1 - rate / 100)));
  });
  //할인율 값 입력시 할인된금액 값 자동변경
  $(".rate").keyup(function () {
    const rate = $(this).val();
    const price = $(this).prev().prev().prev().val();
